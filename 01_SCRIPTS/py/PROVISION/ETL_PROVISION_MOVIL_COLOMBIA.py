...
for filename in glob.glob("*MV*.csv"):
...
for x in range(len(files)):
    sql = """DELETE FROM `bbdd_cs_bog_movistar_ciclos`.`tb_asignacion_provision_movil_movistar_colombia`
...
df_prov['FEC_ASIGNA'] = (re.findall("[0-9][0-9]+",files[i])[0])
df_prov['FECHA_ARCHIVO'] = pd.to_datetime(df_prov['FECHA_ARCHIVO'], format="%Y%m%d")
df_prov['FEC_ASIGNA'] = pd.to_datetime(df_prov['FEC_ASIGNA'], format="%Y%m%d")
df_prov['FEC_CIERRE'] = pd.to_datetime(df_prov['FEC_CIERRE'], format="%d/%m/%Y")


to_sql(df_prov, "tb_asignacion_provision_movil_movistar_colombia", connection,'append',None,10000)
